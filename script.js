// ---------- App State ----------
let appState = { ambientStarted: false };
...
playBtn.addEventListener("click", () => {
  if (!appState.ambientStarted) {
    ambient.play().catch(() => console.log("Ambient blocked until user interacts"));
    appState.ambientStarted = true;
    playBtn.style.display = "none"; // hide button after first click
  }
});
...
if (hero) {
  ...
  window.addEventListener("mousemove", e => {
    ...
    ripples.push({ x: e.clientX - rect.left, y: e.clientY - rect.top, radius: 0, speed: 1.5, alpha: 0.4 });
